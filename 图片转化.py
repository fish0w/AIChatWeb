...
def imageprocess(img_url, x):
    original_image = Image.open(img_url)

    # 调整图片尺寸到192x192像素
    resized_image = original_image.resize((x, x), Image.ANTIALIAS)

    # 保存新尺寸的图片
    resized_image.save(img_url[:img_url.rfind(".")] + f"-{x}*{x}.png", 'PNG')
...
def process_directory(directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".png"):
            ...
            rounded = add_rounded_corners(im)
            save_path = os.path.join(new_directory, filename)
            rounded.save(save_path)
...
def convert_to_transparent_bw(im):
    """
    转换图像为透明背景的黑白图像
    :param im: 原始图像
    :return: 返回透明背景的黑白图像
    ...
    for item in datas:
        # 转换为灰度
        gray = int(0.3 * item[0] + 0.59 * item[1] + 0.11 * item[2])
        # 如果是彩色背景（非透明），则设置为透明
        if item[3] > 0:
            new_data.append((gray, gray, gray, 255))
        else:
            new_data.append((gray, gray, gray, 0))
...
def process_directory(directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".png"):
            ...
            bw_transparent = convert_to_transparent_bw(im)
            save_path = os.path.join(new_directory, filename)
            bw_transparent.save(save_path)
...
if __name__ == '__main__':
    # 提取()
    image="/Users/fish052/Downloads/科创AI/Square310x310Logo.png"
    ...
    # directory_path = "/Users/fish052/Downloads/科创AI"
    #
    #
    # process_directory(directory_path)
# 示例
    directory_path = "/Users/fish052/Downloads/科创AI/new"
